...
const matchesQuery = (text: string, query: string): boolean => {
  ...
  // Check if any significant token is directly inside the text as a whole word or substring
  ...
  if (significantTokens.length > 0) {
    return significantTokens.some(token => lowerText.includes(token));
  }
  
  return queryTokens.some(token => lowerText.includes(token));
};
...
export default function LibrariesDashboard({
  ...
  const filteredChannels = LEARNING_CHANNELS.filter(ch =>
    matchesQuery(ch.name, query) ||
    matchesQuery(ch.category, query) ||
    matchesQuery(ch.description, query) ||
    ch.badges.some(b => matchesQuery(b, query))
  );
    ...
    >
      ...
      <div className={`flex flex-col xl:flex-row xl:items-center justify-start gap-3 border-b-2 pb-3 mb-4 ${isLight ? 'border-gray-200' : 'border-[#121c38]'}`}>
        ...
        <div className="flex flex-wrap gap-1.5 sm:gap-2">
          ...
          <button
            id="tab-hackathons"
            onClick={() => { setActiveTab('hackathons'); setQuery(''); }}
          ...
          <button
            ...
            onClick={() => { setActiveTab('youtubeTeachers'); setQuery(''); }}
            className={`px-3 py-1.5 font-mono text-xs border uppercase transition-all rounded-none cursor-pointer flex items-center gap-1.5 ${
          ...
          <button
            id="tab-channels"
            onClick={() => { setActiveTab('channels'); setQuery(''); }}
            className={`px-3 py-1.5 font-mono text-xs border uppercase transition-all rounded-none cursor-pointer flex items-center gap-1.5 ${
              activeTab === 'channels'
          ...
          <button
            id="tab-tools-skills"
            onClick={() => { setActiveTab('tools-skills'); setQuery(''); }}
            className={`px-3 py-1.5 font-mono text-xs border uppercase transition-all rounded-none cursor-pointer flex items-center gap-1.5 ${
          ...
          <button
            id="tab-certs"
            onClick={() => { setActiveTab('certs'); setQuery(''); }}
            className={`px-3 py-1.5 font-mono text-xs border uppercase transition-all rounded-none cursor-pointer flex items-center gap-1.5 ${
              activeTab === 'certs'
          ...
          <button
            id="tab-bookshelf"
            onClick={() => { setActiveTab('bookshelf'); setQuery(''); }}
            className={`px-3 py-1.5 font-mono text-xs border uppercase transition-all rounded-none cursor-pointer flex items-center gap-1.5 ${
              activeTab === 'bookshelf'
      ...
      {/* Internal search filter for lists */}
      <div className="relative mb-5 font-mono">
        ...
        <input
          ...
          placeholder="Search all directory resources & cross-sync tabs..."
          className={`w-full ${isLight ? 'bg-gray-50 border-gray-300 text-slate-800' : 'bg-[#05070c] border-slate-700 text-white'} border pl-10 pr-4 py-2 text-xs placeholder-gray-500 focus:outline-none focus:border-[#eab308]`}
        />

        {/* Dynamic Cross-Tab Search Results Dropdown */}
        {getCrossTabSearchResults.length > 0 && (
          <div className={`absolute z-40 left-0 right-0 mt-1 border-2 p-3 ${isLight ? 'bg-white border-slate-300 shadow-lg text-slate-800' : 'bg-[#060a12] border-[#1e2e54] shadow-2xl text-white'}`}>
            <div className="flex items-center justify-between border-b border-dashed border-slate-800/40 pb-1.5 mb-2">
              <span className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">
                🔍 Cross-Tab Search Results ({getCrossTabSearchResults.reduce((acc, curr) => acc + curr.count, 0)} Matches)
              </span>
              <button 
                onClick={() => setQuery('')}
                className="text-[10px] text-red-400 hover:text-red-500 uppercase font-bold tracking-wider cursor-pointer"
              >
                Clear [X]
              </button>
            </div>
            ...
            <div className="space-y-1.5 max-h-[220px] overflow-y-auto custom-scrollbar">
              {getCrossTabSearchResults.map((match) => {
                const isActive = activeTab === match.tabId;
                return (
                  <div 
                    key={match.tabId}
                    onClick={() => {
                    ...
                    className={`p-2 flex items-center justify-between border cursor-pointer transition-all ${
                      isActive
                        ? isLight
                        ...
                        : isLight
                          ? 'border-gray-150 bg-gray-50 hover:bg-gray-100 text-slate-700'
                          : 'border-slate-850 bg-[#070c14] hover:bg-slate-900 text-gray-300 hover:border-slate-700'
                    }`}
                  >
                    <div className="flex flex-col gap-0.5 max-w-[70%]">
                      <div className="flex items-center gap-2">
                        <span className={`px-1.5 py-0.5 text-[8.5px] uppercase font-bold border ${
                          isActive
                      ...
                      {match.sampleItems.length > 0 && (
                        <p className="text-[9.5px] text-gray-500 italic truncate font-sans">
                          e.g. {match.sampleItems.join(', ')}
                        </p>
                      )}
                    </div>
                    ...
                    <button
                      type="button"
                      className={`text-[9.5px] font-bold uppercase px-2 py-1 flex items-center gap-1.5 transition-all ${
                        isActive
                          ? 'text-emerald-400'
                          : isLight
                            ? 'bg-slate-200 text-slate-800 border-gray-300'
                            : 'bg-slate-850 text-gray-300 hover:bg-emerald-500 hover:text-black hover:border-emerald-500'
                      }`}
                    >
                      {isActive ? 'Active View' : 'Switch Tab ➔'}
                    </button>
                  </div>
                );
              })}
      ...
      {activeTab === 'channels' && (
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4.5 custom-scrollbar overflow-y-auto max-h-[550px] pr-1">
            {filteredChannels.slice(0, isMobile && !showAllChannels ? 4 : undefined).map((plat) => (
              <div key={plat.name} className={`border-2 hover:border-cyan-500/40 p-4.5 flex flex-col justify-between font-mono text-xs transition duration-200 ${isLight ? 'bg-white border-gray-200 hover:border-cyan-500/65 text-slate-850' : 'bg-[#090f1e] border-[#121c38] hover:border-cyan-500/40 text-slate-300'}`}>
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between items-start gap-1">
                    ...
                    {toggleBookmark && isBookmarked && (
                      <button
                        ...
                        title={isBookmarked(plat.name, 'studyPortal') ? 'Remove portal bookmark' : 'Bookmark study portal'}
                      >
                        <CustomBookmarkIcon className={`w-3.5 h-3.5 ${isBookmarked(plat.name, 'studyPortal') ? 'text-yellow-400 fill-yellow-400' : ''}`} />
                      </button>
                    )}
                  </div>
                ...
                <div className={`mt-4 pt-3 border-t space-y-2.5 ${isLight ? 'border-gray-200' : 'border-[#121c38]'}`}>
                  <div className="flex flex-wrap gap-1">
                    {plat.badges.map((b) => (
                      <span key={b} className={`text-[9px] px-1.5 py-0.5 font-sans font-semibold ${isLight ? 'text-cyan-700 bg-cyan-50/70 border border-cyan-200' : 'text-gray-400 bg-black/60'}`}>#{b}</span>
                    ))}
                  </div>
            ...
            {filteredChannels.length === 0 && (
              <div className={`col-span-3 text-center font-mono py-12 ${isLight ? 'text-slate-500' : 'text-gray-500'}`}>
                No learning portals matched "{query}".
              </div>
            )}
          </div>
