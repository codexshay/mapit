    ...
    <div className={`min-h-screen p-4 md:p-8 font-mono transition-colors ${isLight ? "bg-[#f8fafc] text-slate-900" : "bg-[#05070e] text-zinc-100"}`}>
      ...
      <header className={`max-w-7xl mx-auto mb-8 border-2 p-5 md:p-8 transition-all ${isLight ? "bg-white border-slate-200 text-slate-900 shadow-sm" : "bg-zinc-950 border-zinc-800 text-white shadow-2xl"}`}>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b-2 border-zinc-800 pb-6 mb-6">
          ...
          <div className={`flex items-center gap-3 text-xs px-4 py-3 border shrink-0 font-mono ${isLight ? "bg-slate-50 border-slate-300 text-slate-800" : "bg-zinc-900 border-zinc-700 text-zinc-400"}`}>
            <Linkedin className={`w-6 h-6 shrink-0 ${isLight ? "text-slate-900" : "text-white"}`} />
            <div>
              <div className={`font-bold uppercase ${isLight ? "text-slate-900" : "text-white"}`}>LinkedIn Network Directory</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
          ...
          <div className="md:col-span-4 space-y-1.5">
            <div className="flex items-center justify-between">
              <label className={`text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 ${isLight ? "text-slate-700" : "text-zinc-300"}`}>
            ...
            <select
              ...
              className={`w-full border rounded-none px-3.5 py-2.5 text-xs font-mono focus:outline-none transition-all ${isLight ? "bg-white border-slate-300 text-slate-900 focus:border-slate-800" : "bg-black border-zinc-700 text-zinc-200 focus:border-white"}`}
            >
              ...
              {JOB_PORTALS.map((portal) => {
                ...
                return (
                  <a
                    ...
                    className={`w-10 h-10 border flex items-center justify-center transition-all cursor-pointer shrink-0 ${isLight ? "border-slate-300 bg-white hover:bg-slate-900 hover:text-white text-slate-900 shadow-xs" : "border-zinc-700 bg-zinc-900 hover:bg-white hover:text-black text-zinc-200 shadow-sm"}`}
                  >
                    {portal.icon}
                  </a>
                );
        ...
        <div className="mt-4 pt-3 border-t border-zinc-800">
          <div className="flex items-center justify-between mb-2">
            <span className={`text-xs font-mono font-bold uppercase tracking-wider ${isLight ? "text-slate-700" : "text-zinc-300"}`}>
              ...
              <button
                ...
                className={`px-2.5 py-1 text-xs font-mono font-bold border transition-all cursor-pointer ${
                  selectedLetter === letter
                    ...
                    : (isLight ? 'bg-white text-slate-700 border-slate-300 hover:bg-slate-100' : 'bg-zinc-900 text-zinc-400 border-zinc-800 hover:text-white hover:border-zinc-600')
                }`}
              >
                {letter}
              </button>
